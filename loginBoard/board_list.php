 ...
 */
    ...
    // 1.0 리스트 생성
    // 1.1 DB 연결하는 php 파일 불러오기 -> 1번 불러오면 OK
    ...
    // 전체 게시글 개수 확인
    $allPostCount_sql = "select count(*) as pocnt from board where board_pid=0";
    $allPost_query = mysql_fetch_row(mysql_query($allPostCount_sql));
...
<table id="listTable">
    ...
    <?php
    // 댓글 여부 판단 SQL
    $sql = "select * from board where board_pid = 0 order by reg_date desc LIMIT ".$limit.", ".$list;
